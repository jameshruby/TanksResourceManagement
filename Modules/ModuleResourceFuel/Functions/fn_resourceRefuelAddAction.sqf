...
if (_missingFuel >= 0) then {
	...
	_actionId = [
		...
		{ _target setFuel (fuel _target + (_arguments select 0)) },				//_caller = _this select 1;  // Code executed when action starts
		{ _target setFuel (fuel _target + (_arguments select 0)) }, //      _target setFuel (fuel _target + )                                                                        // Code executed on every progress tick
		{},//TitleText ["	"..VTOL refueled" ", "PLAIN DOWN",0.5];	 //_this call MY_fnc_hackingCompleted                                                // Code executed on completion
		{[_target, _target getVariable "actionId"] call BIS_fnc_holdActionRemove;_actionId = _target call TM_fnc_resourceRefuelAddAction;  _target setVariable ["actionId", _actionId]}, //  Code executed on interrupted
